 ...
 */
class Jyxo_StringTest extends PHPUnit_Framework_TestCase
{
	/**
	...
	{
		$cut = Jyxo_String::cutWords($string, $max, $etc);
	...
	{
		$cut = Jyxo_String::cut($string, $max, $etc);
		// &hellip; has length of 1
	...
	{
		$this->assertSame(-662733300, Jyxo_String::crc('test'));
		$this->assertSame(-33591962, Jyxo_String::crc('žluťoučký kůň příšerně úpěl ďábelské ódy'));
	}
	...
	public function testFixLineEnding()
	{
		...
		foreach ($tests as $test) {
			$this->assertEquals("test\nžlutý\n", Jyxo_String::fixLineEnding($test));
			$this->assertNotEquals($test, Jyxo_String::fixLineEnding($test));
		}
		$this->assertEquals("test\nžlutý\n", Jyxo_String::fixLineEnding("test\nžlutý\n"));
		...
		foreach ($tests as $test) {
			foreach (array("\n", "\r", "\r\n") as $ending) {
				$this->assertEquals(sprintf('test%1$sžlutý%1$s', $ending), Jyxo_String::fixLineEnding($test, $ending));
				$this->assertNotEquals("test\nžlutý\r\n", Jyxo_String::fixLineEnding($test, $ending));
			}
		}
	}
	...
	public function testObfuscateEmail()
	{
		...
		$this->assertEquals('example&#64;example.com', Jyxo_String::obfuscateEmail($email));
		$this->assertEquals('example&#64;<!---->example.com', Jyxo_String::obfuscateEmail($email, true));
	}
	...
	{
		$this->assertEquals('žlutý kůň', Jyxo_String::lcfirst('Žlutý kůň'));
		$this->assertEquals('žlutý kůň', Jyxo_String::lcfirst('žlutý kůň'));
	}
	...
	{
		$this->assertEquals('test &amp; test', Jyxo_String::escape('test & test'));
		$this->assertEquals('&quot;test&quot; &amp; &#039;test&#039;', Jyxo_String::escape('"test" & \'test\''));
		$this->assertEquals('&quot;test&quot; &amp; \'test\'', Jyxo_String::escape('"test" & \'test\'', ENT_COMPAT));
		$this->assertEquals('"test" &amp; \'test\'', Jyxo_String::escape('"test" & \'test\'', ENT_NOQUOTES));
		$this->assertEquals('test &amp; test', Jyxo_String::escape('test &amp; test'));
		$this->assertEquals('test &amp;amp; test', Jyxo_String::escape('test &amp; test', ENT_QUOTES, true));
	}
	...
	{
		$this->assertEquals('11 B', Jyxo_String::formatBytes(10.5));
		$this->assertEquals('11 B', Jyxo_String::formatBytes(10.5, '.'));
		$this->assertEquals('11 B', Jyxo_String::formatBytes(10.5, '.', ','));

		$this->assertEquals('1,0 GB', Jyxo_String::formatBytes(1073741824));
		$this->assertEquals('1.0 GB', Jyxo_String::formatBytes(1073741824, '.'));
		$this->assertEquals('10,240.0 PB', Jyxo_String::formatBytes(11805916207174113034240, '.', ','));
	}
}
