 ...
 */
class Jyxo_Time_ComposerTest extends PHPUnit_Framework_TestCase
{
	/**
	 * Tests invalid data.
	 */
	...
	{
		$composer = new Jyxo_Time_Composer();
		...
		foreach ($units as $unit => $tests) {
			foreach ($tests as $test) {
				try {
					...
					$this->assertInstanceOf('Jyxo_Time_ComposerException', $e);
					$this->assertSame(
						constant('Jyxo_Time_ComposerException::' . strtoupper($unit)),
						$e->getCode(),
		...
		try {
			...
			$this->assertInstanceOf('Jyxo_Time_ComposerException', $e);
			$this->assertSame(Jyxo_Time_ComposerException::NOT_COMPLETE, $e->getCode());
		}
		...
		foreach ($tests as $test) {
			try {
				...
				$this->assertInstanceOf('Jyxo_Time_ComposerException', $e);
				$this->assertSame(
					Jyxo_Time_ComposerException::INVALID,
					$e->getCode(),
