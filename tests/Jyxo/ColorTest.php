 ...
 */
class Jyxo_ColorTest extends PHPUnit_Framework_TestCase
{
	/**
	 * The whole test.
	 */
	...
	{
		$tests = array('#000000', '#000', '000000', 0, array(0, 0, 0), new Jyxo_Color('#000000'));
		foreach ($tests as $test) {
			$color = new Jyxo_Color($test);
		...
		$color = new Jyxo_Color();
		$color->setRed('00')
			->setGreen('00')
			->setBlue('00');
		$this->assertEquals('000000', $color->getHex());

		$tests = array('#ffffff', '#FfFFffF', '#fff', 'ffffff', 0xFFFFFF, array(255, 255, 255), array(260, 320, 1024), new Jyxo_Color('#ffffff'));
		foreach ($tests as $test) {
			$color = new Jyxo_Color($test);
		...
		$tests = array('#239416', '239416', 0x239416, array(35, 148, 22), new Jyxo_Color('#239416'));
		foreach ($tests as $test) {
			$color = new Jyxo_Color($test);
		...
		$tests = array('#22FF66', '#22Ff66', '#2f6', '2F6', '22FF66', '22fF66', 0x22FF66, array(34, 255, 102), new Jyxo_Color('#22FF66'));
		foreach ($tests as $test) {
			$color = new Jyxo_Color($test);
