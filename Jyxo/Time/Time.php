 ...
 */
class Jyxo_Time_Time implements Serializable
{
	/**
	 * Second.
	 *
	 ...
	 *
	 * @param string|integer|Jyxo_Time_Time|DateTime $time Date/time definition
	...
	public function __construct($time, $timeZone = null)
	{
		if (!is_object($time)) {
			...
			if (is_numeric($time)) {
				...
				throw new InvalidArgumentException('Provided date/time must be a number, Jyxo_Time_Time or DateTime instance or a parameter compatible with PHP function strtotime().');
			}
