...
/**
 * Mail parsing class.
 * Based on Mail_IMAPv2 class (c) Copyright 2004-2005 Richard York
 *
 * @category Jyxo
 * @package Jyxo_Mail
 * @subpackage Parser
...
class Jyxo_Mail_Parser
{
	 ...
	 *
	 * @throws Jyxo_Mail_Parser_EmailNotExistException If no such email exists
	 */
	private function checkIfParsed()
	{
		...
		} catch (Jyxo_Mail_Parser_EmailNotExistException $e) {
			throw $e;
		}
	}
	...
	private function setStructure(array $subparts = null, $parentPartId = null, $skipPart = false, $lastWasSigned = false)
	{
		...
		if (null === $subparts) {
			...
			if (!$this->structure['obj']) {
				throw new Jyxo_Mail_Parser_EmailNotExistException('Email does not exist');
			}
		}
	...
	private function getDefaultPid($mimeType = 'text/html', $attempt = 1)
	{
		...
		if (1 === $attempt) {
			...
			return 1;
			// throw new Jyxo_Mail_Parser_DefaultPartIdNotExistException('Default part Id does no exist.');
		}
	}
	 ...
	 * @param string $pid Part Id
	 * @return string
	 * @throws Jyxo_Mail_Parser_PartNotExistException If there is no such
	 */
	private function getRawHeaders($pid = null)
	{
		...
		if (empty($rawHeaders)) {
			throw new Jyxo_Mail_Parser_PartNotExistException('No such part exists.');
		}
	 ...
	 * @return array
	 * @throws Jyxo_Mail_Parser_EmailNotExistException If no such email exists
	 * @throws Jyxo_Mail_Parser_PartNotExistException If no such part exists
	 */
	public function getBody($pid = '1', $mode = self::BODY, $mimeType = 'text/html', $attempt = 1)
	{
		...
		$key = array_search($pid, $this->structure['pid']);
		if (false === $key) {
			throw new Jyxo_Mail_Parser_PartNotExistException('Requested part does not exist');
		}
	...
	private function convertToUtf8($string, $charset = '')
	{
		...
		if ('default' === $charset || 'us-ascii' === $charset || empty($charset)) {
			$charset = Jyxo_Charset::detect($string);
		}

		return Jyxo_Charset::convert2utf($string, $charset);
	}
}
