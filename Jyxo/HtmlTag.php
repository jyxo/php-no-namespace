...
final class Jyxo_HtmlTag
{
			...
			if (isset(self::$attrs[$name])) {
				...
				if ($this->isRequiredAttr($this->tag, $name) || $notEmpty) {
					...
					if (!isset($this->noEncode[$name])) {
						$value = Jyxo_String::escape($value);
					}
	...
	public function content()
	{
		...
		if (!$this->isEmptyElement) {
			...
			if ($hasValue || $hasText) {
				...
				$buff .= $noEncode ? $text : Jyxo_String::escape($text);
			}
		}
	 ...
	 *
	 * @param Jyxo_HtmlTag $element Child element to be added
	 * @return Jyxo_HtmlTag
	 */
	public function addChild(Jyxo_HtmlTag $element)
	{
