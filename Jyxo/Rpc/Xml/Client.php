 ...
 */
class Jyxo_Rpc_Xml_Client extends Jyxo_Rpc_Client
{
	/**
	 ...
	 * @throws Jyxo_Rpc_Xml_Exception On error
	 */
	public function send($method, array $params)
	{
		...
		try {
			...
			throw new Jyxo_Rpc_Xml_Exception($e->getMessage(), 0, $e);
		}
		...
		if ((is_array($response)) && (isset($response['faultString']))) {
			throw new Jyxo_Rpc_Xml_Exception(preg_replace('~\s+~', ' ', $response['faultString']));
		}
