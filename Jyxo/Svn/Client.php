...
class Jyxo_Svn_Client
{
	 ...
	 * @return Jyxo_Svn_Result
	 * @throws Jyxo_Svn_Exception On execute error
	 */
	protected function callSvn($action, $user, $password, $params)
	{
		try {
			...
			try {
				...
				$shell = new Jyxo_Shell_Client();
				$shell->exec($command, $status);

				return new Jyxo_Svn_Result($action, $shell->getOut(), $status);

			} catch (Jyxo_Shell_Exception $e) {
				throw $e;
			}

		} catch (Exception $e) {
			throw new Jyxo_Svn_Exception(sprintf('SVN %s failed: %s', $action, $e->getMessage()), 0, $e);
		}
	}
